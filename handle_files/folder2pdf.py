...
class MyPdfZip(object):
    ...
    def composite_files(self):
        ...
        for parent, dirnames, filenames in os.walk(rootdir):
            ...
            for filename in filenames:
                ...
                f_obj.file_dir = parent
                oldfilepath = os.path.join(parent + '/' + filename)
                # oldfilepath = oldfilepath.replace('//', '/')
                # print('oldfilepath:{}'.format(oldfilepath))
                
                if filename[-3:] == 'jpg':
                    self.file_name_pdf.append(oldfilepath)
                elif filename[-3:] == 'tif':
                    self.file_name_pdf.append(oldfilepath)
                else:
                    self.file_name_zip.append(oldfilepath)
                
            
            # 检查文件是否能打开，如果不能打开，则不生成PDF
            for f in self.file_name_pdf:
                # 是否处理png 修改为jpg 的文件（如果处理判断条件！=jpeg or ！=tiff）
                f_format = imghdr.what(f)
                # print('f_format is {}:'.format(f_format))
                if f_format != 'jpeg' and f_format != 'tiff':
                    print('该文件{} 无法打开或者格式不对, 格式为：{}'.format(f, f_format))
                    print('该PDF{} 无法生成因为有文件打不开'.format(f_obj.file_name))
                    self.file_name_pdf = []

            # 生成PDF 文件
            # if f_obj.file_type == 'pdf' and self.file_name_pdf != []: 
            if self.file_name_pdf != []: 
                f_obj.file_type = 'pdf'
                f_obj.file_name = name_str  + '.pdf'
                if not self.query_sql_exist(f_obj):
                    # 记录源文件的父名字，名字，大小，md5，类型等等
                    for origin_file in self.file_name_pdf:
                        # 初始化源文件 文件对象
                        o_obj = MyOriginFile()
                        o_obj.p_name = f_obj.file_name
                        o_obj.file_dir = origin_file
                        o_obj.file_name = origin_file[len(parent)+1:]
                    ...
                    try:
                        # 开始将jpg 和 tif 转换成pdf
                        self.generate_pdffile(f_obj)
                        # 开始为PDF 文件添加书签
                        self.pdf_add_bookmark(f_obj)
                        # 移动文件
                        self.shutil_move(parent, f_obj)
                    except:
                        # 错误信息写入日志
    ...
    def pdf_add_bookmark(self, obj):
        ...
        print('{}增加书签完成'.format(obj.file_name))
        


    def generate_zipfile(self, obj):
        # 压缩其他文件
        z = zipfile.ZipFile(obj.file_name, 'w', zipfile.ZIP_DEFLATED)
        for zip_file in self.file_name_zip:
            z.write(zip_file)
        z.close()
        # # 打印日志信息
    ...
    def conn_postgresql(self):
        self.conn = psycopg2.connect(database='test_class', user='postgres', password='123456')
